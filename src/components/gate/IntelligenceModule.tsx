...
export function IntelligenceModule({ icon, title, subtitle, delay = 0 }: IntelligenceModuleProps) {
  ...
  return (
    ...
    >
      <div
        className="relative p-6 md:p-8 rounded-2xl border border-border/20 backdrop-blur-xl overflow-hidden transition-all duration-500"
        style={{
          background: 'linear-gradient(135deg, hsl(var(--card) / 0.6) 0%, hsl(var(--card) / 0.3) 100%)',
          boxShadow: hovered
            ? '0 0 40px hsl(var(--gold) / 0.1), inset 0 1px 0 hsl(var(--gold) / 0.15)'
            : 'inset 0 1px 0 hsl(var(--crystal-silver) / 0.06)',
          borderColor: hovered ? 'hsl(var(--gold) / 0.3)' : undefined,
        }}
      >
          ...
          {[...Array(3)].map((_, i) => (
            <div
              ...
              style={{
                ...
                left: 0,
                right: 0,
                background: `linear-gradient(90deg, transparent, hsl(var(--gold) / 0.12), transparent)`,
